...
public class Player : MonoBehaviour
{
  ...
  //  bool isShop;    //쇼핑중일때
    ...
    {   //키입력 이벤트
        ...
        sDown = Input.GetButtonDown("Putitem");
    }
    ...
    void Move() //이동
    {
        ...
        if (!isBorder)  //벽에 닿았을때   > 176줄에 해당코드
        {
            ...
            anim.SetBool("isRun", moveVec != Vector3.zero); //파라미터값 설정
            anim.SetBool("isWalk", wDown); //파라미터값 설정
    }
    ...
    void Jump() //점프
    {
        if(jDown && moveVec == Vector3.zero && !isJump && !isSwap)    //점프키를 누르고 isJump가 false일때, isSwap이 false일때
        {
            ...
            anim.SetTrigger("doJump");  //애니메이션 트리거
            isJump = true;
        }
    }
    void Swap() //무기교체
    ...
    void Interation()   //E키를 눌렀을 때 실행되는 상호작용
    {
        if(iDown && nearObject != null && !isJump)  //E키가 눌렸고, 가까운물체가 존재하고, isJump가 아닐때
        {
            ...
            {
                save = nearObject;
                Item item = nearObject.GetComponent<Item>();   //Item 컴포넌트를 받아옴 
                int weaponIndex = item.value;   //item의 value값을 대입함
                hasWeapons[weaponIndex] = true;     //현재 착용중인 무기배열의 값을 활성화 시킴 
            ...
            else if (nearObject.tag == "Shop")  //접촉한 물체가 상점이면
            {
                ...
                shop.Enter(this);   //플레이어 정보를 집어 넣어줌 //this = Player
               // isShop = true;
            }
